...
(function ($) {
    ...
    function setBuildsHistory(nodeName)
    {
        $("#buildHistoryTable tbody").empty();
        a.getBuildHistory(nodeName, function (t) {
            let response = t.responseObject();
            $.each(response, function (i, item) {
                console.log(item);
                $("#buildHistoryTable tbody").append("<tr><td>" + item.jobName + "</td><td>" + item.number + "</td><td><a href=\"" + item.buildUrl + "\">link</a></td><td>" + item.startedAt + "</td><td>" + item.endedAt + "</td><td>" + item.result + "</td></tr>");
            });
        });
    }
